...
currIndex = bulktext.find("TOSS-UP")
prevIndex = currIndex
while currIndex > 0:
    #Find the bonus first since you're doing from curr up to "bonus"
    currIndex = bulktext.find("BONUS", currIndex)
    questions.append(bulktext[prevIndex:currIndex])
    prevIndex = currIndex
    #This section gets the Bonus question
    currIndex = bulktext.find("TOSS-UP", currIndex)
    questions.append(bulktext[prevIndex:currIndex])
    prevIndex=currIndex
...
for questIndex in range(len(questions)):
    ...
    if topicxml.text[:2] == '1)': #Hardcode out number one misprint
        topicxml.text = 'CHEMISTRY'
